...
// datacontainer.map((arr, index) => (<CardTamplete key={index} darkmode={darkmode} data={arr} openModal={openModal} />))
        ...
        <div className={`mx-auto max-w-[1440px] min-h-screen overflow-x-hidden ${showModal ? "noscroll" : ""}`}>
            ...
            {issearcheddata &&
                <div className='mx-auto max-w-[1100px] w-full flex items-center flex-wrap lg:mt-14 mt-4'>
                    <h2 className={`text-3xl mx-2 my-2 font-extrabold font-[Montserrat] ${darkmode ? "text-[#FFFFFF]" : "text-color-2-dark"}`}>{searchitem}</h2>
                    <div className='flex gap-2 flex-wrap my-2 mx-2'>
                        {tagarray.map((tag, index) => (
                            <span key={index} className={`lg:text-[14px] text-[10px] font-medium rounded-md bg-transparent cursor-pointer ${darkmode ? "text-[#FFFFFF] border-[1.5px] border-[#FFFFFF] hover:bg-color-2-dark" : "text-[#4F4F4F] border-[1.5px] border-[#4F4F4F] hover:bg-color-3-dark"} lg:px-4 lg:py-2 px-2 py-1 font-[Montserrat]`}>{tag}</span>
                        ))}
                    </div>
                </div>
            }
            {isLoading ? <Loadinganimation /> : (issearcheddata ? searchdata.map((arr, index) => (<CardTamplete key={index} darkmode={darkmode} data={arr} openModal={openModal} />)) : data.map((arr, index) => (<CardTamplete key={index} darkmode={darkmode} data={arr} openModal={openModal} />)))}
            {!isLoading &&
